...
if not os.path.exists('./input_mnist'):
    os.mkdir('./input_mnist')
...
class DrawModel(nn.Module):
    ...
    def filterbank(self,gx,gy,sigma2,delta):
        rng = Variable(torch.arange(0,self.N).float().view(1,-1))
        mu_x = self.compute_mu(gx,rng,delta)
        mu_y = self.compute_mu(gy,rng,delta)

        a = Variable(torch.arange(0,self.A).float().view(1,1,-1))
        b = Variable(torch.arange(0,self.B).float().view(1,1,-1))
